...
class DatasetFolder(object):
    ...
    def select_class(self, data, cls, selection):
        ...
        for x in selection:
            if x < len(cls):  # Ensure x is within range
                cls_new.append(cls[x])
            else:
                cls_new = cls
            #     print(f"Warning: Selection index {x} is out of range for cls with length {len(cls)}")

        # Collect data for valid classes
        for cl in cls_new:
