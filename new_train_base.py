...
def main(args):
    ...
    # CSV 파일의 고정 경로
    csv_base_path = "/content/mtunet_sea3/data/data_split/seas"

    # get_dataloader에서 고정 경로로 CSV 파일 참조
    loaders_train = get_dataloader(args, "train", csv_path=os.path.join(csv_base_path, "train.csv"))
    sample_info_val = [args.val_episodes, args.n_way, args.n_shot, args.query]
    loaders_val = get_dataloader(args, "val", csv_path=os.path.join(csv_base_path, "val.csv"))
